...
class Pac extends Connection{
    ...
    public function index(){
      ...
      FROM
        ...
        WHERE
          TC_GROUP IN (SELECT DISTINCT(TC_GROUP) FROM `pit-analytics-2019.PIT_SISTEMAS.PAC_GROUP`)
          AND
          TC_SUBGROUP IN (SELECT DISTINCT(TC_SUBGROUP) FROM `pit-analytics-2019.PIT_SISTEMAS.PAC_GROUP`)
          AND
          TRX_CODE IN (SELECT DISTINCT(TRX_CODE) FROM `pit-analytics-2019.PIT_SISTEMAS.PAC_GROUP`)
      ORDER BY
        BUSINESS_DATE";
